...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        ...
        // total consumers
        $stmt = $conn->prepare("SELECT COUNT(user_id) AS Total_Consumers FROM user_consumer WHERE branchId = 2");
        $stmt->execute();
        $CTotalresults = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // total employees
        $stmt = $conn->prepare("SELECT COUNT(user_id) AS Total_Employees FROM user_employee WHERE branchId = 2");
        $stmt->execute();
        $ETotalresults = $stmt->fetchAll(PDO::FETCH_ASSOC);

        // total consumed (pres + prev)
        $stmt = $conn->prepare("SELECT SUM(cubic_consumed) AS Total_Consumed FROM billing WHERE total_bill != 0 AND branchId = 2");
        $stmt->execute();
        $PresConsumedTotalresults = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $stmt = $conn->prepare("SELECT SUM(pay_amount) AS Total_Pay FROM payment WHERE branchId = 2");
        $stmt->execute();
