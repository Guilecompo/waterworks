...
const showClerkEmployeePage = (page, employeesToDisplay = employees) => {
  ...
  showPaginationNumbersClerk(page, Math.ceil(employeesToDisplay.length / 10));
};
const showPaginationNumbersClerk = (currentPage, totalPages) => {
  ...
  for (let i = startPage; i <= endPage; i++) {
    if (i === currentPage) {
      paginationNumbersHTML += `<span class="active" onclick="goToPageClerk(${i})">${i}</span>`;
    } else {
      paginationNumbersHTML += `<span onclick="goToPageClerk(${i})">${i}</span>`;
    }
  }
...
const goToPageClerk = (pageNumber) => {
  showClerkEmployeePage(pageNumber);
};
  ...
  employeeList.forEach((employee) => {
    html += `
              <tr>
                ...
                <td>
                  <button class="clear" onclick="view_clerk(${employee.user_id})">View</button>  
